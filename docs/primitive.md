...
输入丢帧会显示警告；原生采集检测 3 秒无样本和连续溢出，网页另外在 3 秒提示停滞、6 秒发起停止失效会话。音频异常或被强制终止的进程显示“测试失败”，不会冒充正常结束。
...
单独叫名字时播放缓存的 “I'm here.”；称呼后带请求时，立即播放缓存的 “Let me think for a moment.”（可用 `SPARKIE_QUESTION_REPLY` 配置），同时开始推理。两种确认音频均在会话开始前合成，选择确认语不调用模型；答案文字生成后立刻显示，再通过 Deepgram 英文 TTS 朗读。生成答案期间继续收集讨论；扬声器播放期间仍有回声静音窗口。一个答案完成前再次提问会显示“正忙”，请稍后重新叫醒，不会自动排队。页面停止/关页超时会取消正在进行的推理及音频。
...
唤醒按句首称呼判断，不使用请求词汇或句式白名单，接受 `Sparkie` 和 `Sparky`，支持前置 Hi/Hey/Hello、标点变化和连续叫名。对已复现的问候误转录 `Hi. It's Sparkie.` 做了仅限完整问候句的兼容；不会任意匹配句中提到产品名字。最终转录段之间尚不等待并拼接请求。回复中再次唤醒会记录 `response_busy` 并忽略；取消词会停止当前回复。模拟可标注 bot 来源；本地扬声器模式有静音窗口，Zoom 也在回复和尾音期间替换输入为静音；全双工回声消除仍未实现。
