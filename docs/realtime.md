...
前台提示词要求普通回答为一到两句短句，委派确认一句，任务完成时先说主要结论、最多三句短句，细节留在任务面板，用户要求时再展开。目标明确但部分词未听清时，只委派听清的目标和细节，让后台核对相关的人声转写；不猜姓名、地点、日期或数字，不添加候选解释或臆测的歧义。转写可能延迟、错误或缺失，后台无法确认必要信息时再简短追问；本机直接工具缺少必要参数时先询问。提示词变更在新语音会话生效；这不是转写完整性或识别准确性的保证。

简单的桌面文本文件创建使用 `create_desktop_file`，打开指定网址或本地 HTML 报告使用 `open_website`，直接调用本机能力并显示任务结果，不进入后台队列。创建文件不会覆盖已有文件；打开操作接受 HTTP(S) 或本机已存在的 file:///…html（也支持 .htm），只确认系统启动器接受请求，不声称网页已加载；不接受远程 file 主机或其他本地文件类型，失败会显示具体原因。网页检索、复杂内容生成、其他文件/代码操作仍通过 delegate_task 交给所选后台。
