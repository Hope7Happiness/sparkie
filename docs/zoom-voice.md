...
macOS SDK 7.1.5 的 wake/qa 路径现改用每位参会端的独立音轨，分别送 Deepgram，并保留 Zoom 用户 ID 和显示名。Linux 与 Realtime 路径仍用混音。新版 main 默认启动 Realtime；本轮分用户验收必须显式传 --response-mode wake 或 qa。原生代码更新后必须重新构建：
